...
CREATE TABLE "registration_2_role" (
	"registration_id" bigint REFERENCES registration_data (registration_id) ON UPDATE CASCADE ON DELETE CASCADE,
	"role_id" bigint REFERENCES product (role_id) ON UPDATE CASCADE,
	UNIQUE(registration_id, role_id),
	CONSTRAINT registration_2_role_pkey PRIMARY KEY (registration_id, role_id) 
) WITH (
  OIDS=FALSE
);
